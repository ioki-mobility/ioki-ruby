...
RSpec.describe Ioki::OperatorApi do
  ...
  describe '#driver_overview(product_id)' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/products/0815/drivers/overview')
        result_with_index_data
      end

      expect(operator_client.driver_overview('0815', options))
        .to all(be_a(Ioki::Model::Operator::Driver))
    end
  end
  ...
  describe 'station_batch_destroy(product_id, station_batch)' do
    ...
    it 'calls request on the client with expected params' do
      ...
      expect(operator_client.create_station_batch_destroy('0815', station_batch, options))
        .to be_nil
    end
  end
  ...
  describe 'task_list_batch_destroy(product_id, task_list_batch)' do
    ...
    it 'calls request on the client with expected params' do
      ...
      expect(operator_client.create_task_list_batch_destroy('0815', task_list_batch, options))
        .to be_nil
    end
  end
  ...
  describe '#line_overview(product_id)' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/products/0815/lines/overview')
        result_with_index_data
      end

      expect(operator_client.line_overview('0815', options))
        .to all(be_a(Ioki::Model::Operator::Line))
    end
  end
  ...
  describe '#users_ride_templates(provider_id, user_id, ...)' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/providers/0815/users/01/ride_templates')
        result_with_index_data
      end
  ...
  describe '#create_reporting_aggregation_series(scope, name, aggregation_name, query)' do
    ...
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s)
          .to eq('operator/reporting/report/scopes/myscope/reports/myname/aggregations/ride_counts/series')
        expect(params[:method]).to eq(:post)
        expect(params[:body]).to eq({ data: query.serialize(:create, format: :json) })
        [result_with_reporting_aggregation, full_response]
      end

      aggregation_result = operator_client.create_reporting_aggregation_series(
        'myscope',
        'myname',
        'ride_counts',
        query,
  ...
  describe '#create_reporting_aggregation_totals(scope, name, aggregation_name, query)' do
    let(:query) do
      Ioki::Model::Operator::Reporting::ReportAggregationTotalsQuery.new(
        start_time: DateTime.parse('2026-04-01T00:00:00Z'),
        end_time:   DateTime.parse('2026-05-01T00:00:00Z'),
        filters:    [
          Ioki::Model::Operator::Reporting::ReportAggregationFilterParam.new(
    ...
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s)
          .to eq('operator/reporting/report/scopes/myscope/reports/myname/aggregations/ride_counts/totals')
        expect(params[:method]).to eq(:post)
        expect(params[:body]).to eq({ data: query.serialize(:create, format: :json) })
        [result_with_reporting_aggregation, full_response]
      end

      aggregation_totals = operator_client.create_reporting_aggregation_totals(
        'myscope',
        'myname',
        'ride_counts',
        query,
  ...
  describe '#admin_notification(admin_notification_id)' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/admin/admin_notifications/0815')
        [result_with_data, full_response]
      end

      expect(operator_client.admin_notification('0815', options))
  ...
  describe '#admin_notifications()' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/admin/admin_notifications')
        [result_with_data, full_response]
      end
  ...
  describe '#delete_admin_notification(admin_notification_id)' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/admin/admin_notifications/0815')
        expect(params[:method]).to eq(:delete)
      ...
      expect(operator_client.delete_admin_notification('0815', options))
        .to be_a(Ioki::Model::Operator::AdminNotification)
    end
  end

  describe '#create_admin_notification_read_confirmation(options)' do
    let(:admin_notification_bulk) { Ioki::Model::Operator::AdminNotificationBulk.new }

    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/admin/admin_notifications/read_confirmations')
        expect(params[:method]).to eq(:post)
        [result_with_data, full_response]
      end

      expect(operator_client.create_admin_notification_read_confirmation('0815', admin_notification_bulk, options))
        .to be_a(Ioki::Model::Operator::AdminNotification)
    end
  end

  describe '#delete_admin_notification_read_confirmation(options)' do
    let(:admin_notification_bulk) { Ioki::Model::Operator::AdminNotificationBulk.new }
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/admin/admin_notifications/read_confirmations')
      ...
      expect(operator_client.delete_admin_notification_read_confirmation('0815', admin_notification_bulk, options))
        .to be_a(Ioki::Model::Operator::AdminNotification)
    end
  end
