...
RSpec.describe Ioki::OperatorApi do
  ...
  describe '#reporting_scopes' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/reports/scopes')
        result_with_index_data
      end
  ...
  describe '#reporting_scope_structure(scope_id)' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/reports/scopes/0815/structure')
        [result_with_data, full_response]
      end
  ...
  describe '#reporting_report_type_summary(scope, local_year, name, period_identifier, version)' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/reports/scopes/0815/report_types/a-name')
        [result_with_data, full_response]
      end
  ...
  describe '#reporting_rows' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/reports/data/myscope/myyear/myname/myperiod/myversion/rows')
        result_with_index_data
      end
  ...
  describe '#reporting_report(scope, local_year, name, period_identifier, version)' do
    it 'calls request on the client with expected params' do
      expect(operator_client).to receive(:request) do |params|
        expect(params[:url].to_s).to eq('operator/reports/data/myscope/myyear/myname/myperiod/myversion')
        [result_with_data, full_response]
      end
