...
RSpec.describe Endpoints::Endpoint do
  ...
  describe '#call' do
    let(:headers) { instance_double(Hash, :headers) }
    ...
    let(:client) do
      instance_double(Ioki::Client, :client, build_request_url: url, all_headers: headers, request: request)
    end
    ...
    it 'passes on given params to client#request' do
      expect(client).to receive(:request).with(method: :get, body: 'null', headers: headers, params: params, url: url)
